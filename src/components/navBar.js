...
import { AppBar, Toolbar, Typography, Button, Box, IconButton, Popover, List, ListItem, ListItemIcon, ListItemText } from '@mui/material';
...
function Navbar() {
  ...
  return (
    <>
      <AppBar position="static" sx={{ backgroundColor: '#AC8968',
        //top:120,zIndex:1000 
        }}>
        <Toolbar sx={{ justifyContent: 'space-between' }}>
          {isMobile ? (
                ...
                <Box sx={{ p: 2, width: 200, boxShadow: 3 }}>
                  <List>
                    {menuItems.map((item, index) => (
                      <ListItem button key={index} component={Link} to={item.to} onClick={handleMenuClose}>
            ...
            <Box sx={{ display: 'flex' }}>
              {menuItems.map((item, index) => (
                <Button key={index}  component ={Link}to={item.to} sx={buttonStyle}>
                  {item.icon}
                  {item.text}
                </Button>
              ))}
